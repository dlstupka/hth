...
def write_rankings(path: Path, ranked: list[dict[str, Any]]) -> None:
    fields=["rank","parameter_set_id","parameter_identity_sha256","parameter_schema_version","parameter_grid_ordinal","profile","mean_iou","minimum_iou","mean_edge_error_px","mean_iou_success","failure_count","elapsed_ms_total","parameters_json"]
    with path.open("w",newline="",encoding="utf-8") as h:
        w=csv.DictWriter(h,fieldnames=fields); w.writeheader()
        for r in ranked:
