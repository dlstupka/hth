    ...
    with path.open("w",newline="",encoding="utf-8") as h:
        ...
        for result in ranked:
            ...
            for page in result["pages"]:
                ...
                w.writerow({
                    "run_id":result.get("run_id",""),"parameter_set_id":result["parameter_set_id"],"parameter_identity_sha256":result.get("parameter_identity_sha256",""),"parameter_schema_version":result.get("parameter_schema_version",""),"parameter_grid_sha256":result.get("parameter_grid_sha256",""),"parameter_grid_ordinal":result.get("parameter_grid_ordinal",""),"profile":result.get("profile") or "","rank":result.get("rank",""),"search_rank":result.get("search_rank",""),"requested_search_member":1 if result.get("requested_search_member") else 0,"reference_roles_json":json.dumps(result.get("reference_roles") or []),"historic_reference_json":json.dumps(result.get("historic_reference") or {}),
...
def write_rankings(path: Path, ranked: list[dict[str, Any]]) -> None:
    fields=["rank","search_rank","requested_search_member","reference_roles_json","parameter_set_id","parameter_identity_sha256","parameter_schema_version","parameter_grid_ordinal","profile","mean_iou","minimum_iou","mean_edge_error_px","mean_iou_success","failure_count","elapsed_ms_total","parameters_json"]
    with path.open("w",newline="",encoding="utf-8") as h:
        w=csv.DictWriter(h,fieldnames=fields); w.writeheader()
        for r in ranked:
            s=r["summary"]; w.writerow({"rank":r["rank"],"search_rank":r.get("search_rank",""),"requested_search_member":1 if r.get("requested_search_member") else 0,"reference_roles_json":json.dumps(r.get("reference_roles") or []),"parameter_set_id":r["parameter_set_id"],"parameter_identity_sha256":r.get("parameter_identity_sha256",""),"parameter_schema_version":r.get("parameter_schema_version",""),"parameter_grid_ordinal":r.get("parameter_grid_ordinal",""),"profile":r.get("profile") or "","mean_iou":s["mean_iou"],"minimum_iou":s["minimum_iou"],"mean_edge_error_px":s["mean_edge_error_px"],"mean_iou_success":s.get("mean_iou_success",s["mean_iou"]),"failure_count":s["failure_count"],"elapsed_ms_total":s["elapsed_ms_total"],"parameters_json":canonical_parameters(r["parameters"])})
