"""Gradient Boundary Voting adapter for the detector-agnostic regression framework."""
...
def detect(*, image_bgr: np.ndarray, mask: np.ndarray, parameters: dict[str, Any] | None = None) -> Candidate:
    return run_registered_detector("gradient_vote", image_bgr=image_bgr, mask=mask, parameters=parameters)
