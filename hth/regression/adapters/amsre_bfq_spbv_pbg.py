"""Fusion Gen2 adapter for detector-agnostic regression."""
...
def detect(*, image_bgr: np.ndarray, mask: np.ndarray, parameters: dict[str, Any] | None = None) -> Candidate:
    return run_registered_detector("amsre_bfq_spbv_pbg", image_bgr=image_bgr, mask=mask, parameters=parameters)
