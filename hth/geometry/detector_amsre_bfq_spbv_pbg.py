...
CHILDREN = (
    ("adaptive_multi_scale_radial_edge", detector_adaptive_multi_scale_radial_edge.detect),
...
def _parameters(overrides: dict[str, Any] | None) -> dict[str, float]:
    ...
    if unknown:
        raise ValueError(f"Unknown Fusion Gen2 parameters: {', '.join(unknown)}")
...
def _gray(image_bgr: np.ndarray) -> np.ndarray:
    ...
    if image_bgr.ndim == 2:
        return image_bgr
    raise ValueError(f"Fusion Gen2 expects a 2-D or 3-D image, got {image_bgr.shape}")
...
def debug_images(*, image_bgr: np.ndarray, mask: np.ndarray, parameters: dict[str, Any] | None = None, candidate_corners: list[list[float]] | None = None, verbose: bool = False) -> dict[str, np.ndarray]:
    ...
    images = {
        "fusion-gen2-gradient.png": gradient,
        "fusion-gen2-child-quads.png": child_view,
        "fusion-gen2-selected-quad.png": selected_view,
    }
    if verbose and run["best"] is not None:
        ...
        images["fusion-gen2-side-sources.png"] = side_view
    return images
