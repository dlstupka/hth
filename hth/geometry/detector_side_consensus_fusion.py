...
class SideConsensusFusion:
    method: str
    label: str
    baseline_parameters: dict[str, Any]
    child_calibrations: dict[str, dict[str, Any]]
    children: tuple[tuple[str, Detector], ...]
    child_parameter_defaults: dict[str, dict[str, Any]]
    debug_prefix: str


COMMON_CHILD_CALIBRATIONS: dict[str, dict[str, Any]] = {
    "border_fusion_quad": {
...
def _parameters(family: SideConsensusFusion, overrides: dict[str, Any] | None) -> dict[str, float]:
    values = dict(family.baseline_parameters)
    overrides = overrides or {}
    unknown = sorted(set(overrides) - set(values))
    if unknown:
        raise ValueError(f"Unknown {family.label} parameters: {', '.join(unknown)}")
...
def _gray(image_bgr: np.ndarray, label: str) -> np.ndarray:
    if image_bgr.ndim == 3:
        return cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
    if image_bgr.ndim == 2:
        return image_bgr
    raise ValueError(f"{label} expects a 2-D or 3-D image, got {image_bgr.shape}")
...
def _child_parameters(family: SideConsensusFusion, method: str) -> dict[str, Any]:
    values = dict(family.child_parameter_defaults[method])
    values.update(family.child_calibrations[method]["parameters"])
    return values


def _candidate_summary(
    family: SideConsensusFusion, candidate: Candidate, method: str,
) -> dict[str, Any]:
    return {
        "method": method,
        "parameter_set_id": family.child_calibrations[method]["parameter_set_id"],
...
def _run(
    family: SideConsensusFusion,
    image_bgr: np.ndarray,
    mask: np.ndarray,
    values: dict[str, float],
) -> dict[str, Any]:
    gray = _gray(image_bgr, family.label)
    h, w = gray.shape
    ...
    for method, detector in family.children:
        candidate = detector(
            image_bgr=image_bgr,
            mask=mask,
            parameters=_child_parameters(family, method),
        )
        child_results.append(_candidate_summary(family, candidate, method))
...
def detect(
    ...
    *,
    ...
    values = _parameters(family, parameters)
    run = _run(family, image_bgr, mask, values)
    diagnostics: dict[str, Any] = {
        "parameters": values,
        "child_calibrations": family.child_calibrations,
    ...
    if len(run["available"]) < 2:
        ...
        return Candidate(family.method, None, None, 0.0, 0.0, diagnostics, status="no_candidate")
    best = run["best"]
    if best is None:
    ...
    h, w = _gray(image_bgr, family.label).shape
    bbox = [int(max(0, x1)), int(max(0, y1)), int(min(w, x2)), int(min(h, y2))]
    return Candidate(family.method, bbox, corners.astype(float).tolist(), best["score"], best["score"], diagnostics)
...
def debug_images(
    ...
    *,
    ...
    images = {
        f"{family.debug_prefix}-gradient.png": gradient,
        f"{family.debug_prefix}-child-quads.png": child_view,
        f"{family.debug_prefix}-selected-quad.png": selected_view,
    }
    if verbose and run["best"] is not None:
        ...
        images[f"{family.debug_prefix}-side-sources.png"] = side_view
    return images


__all__ = ["COMMON_CHILD_CALIBRATIONS", "SideConsensusFusion", "debug_images", "detect"]
