#!/usr/bin/env python3
"""Render compact GitHub summaries from canonical chromatic evidence."""
...
def _method_summary(comparison: dict[str, Any]) -> list[str]:
    lines = [
        "### Bounded chromatic method comparison",
        "",
        ...
        "",
        "| Method | Safe pages | Mean cast reduction | Mean luminance-detail correlation | Gate failures |",
        "|---|---:|---:|---:|---|",
    ]
    ...
    for method in comparison["config"]["methods"]:
        ...
        failures = {
            ...
            for gate in (
                "background_cast_reduction",
                "luminance_detail_correlation",
                "chroma_structure_correlation",
                "gamut_clipping",
        ...
        mean_gain = statistics.fmean(
            variant["background_cast_reduction_fraction"] for variant in variants
        ) if variants else 0.0
        mean_correlation = (
            statistics.fmean(variant["luminance_detail_correlation"] for variant in variants)
            if variants else 0.0
        )
        lines.append(
...
def summary_lines(stage: str, payload: dict[str, Any]) -> list[str]:
    if stage == "assess":
        ...
        return [
            "### Chromatic and background-color assessment",
            "",
    ...
    if stage == "validate":
        ...
        return [
            ...
            f"- Mean background-cast reduction: `{aggregate['mean_background_cast_reduction_fraction']:.3f}`",
    ...
    return [
        ...
        f"- Chromatic result: `{payload['chromatic_result_identity']}`",
        "- Pipeline action: `all-pages-continue`",
    ]
