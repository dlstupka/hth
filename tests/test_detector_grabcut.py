...
def test_grabcut_accepts_black_box_parameter_overrides() -> None:
...
def test_grabcut_rejects_unknown_parameter() -> None:
    image = np.zeros((100, 100, 3), dtype=np.uint8)
    mask = np.zeros((100, 100), dtype=np.uint8)
    with self.assertRaises(ValueError, match="Unknown GrabCut parameters"):
