...
class RegressionSummaryTests(unittest.TestCase):
    ...
    def test_builds_manifest_with_winner_baseline_and_outputs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("matching a parameter SHA alone does not imply identical results", text)
            ...
            self.assertIn("| Rank | Last Build | Family ID | Parameter Set ID | Parameter Short Name | Avg IoU | Min IoU | StdDev | Δ Avg IoU | Avg IoU Success | Failures | Discovery Time | Search Space % |", text)
            self.assertIn("| `winner` | `calibrated-winner` | 0.9700 | 0.9100 |", text)
            self.assertIn("12s | 4.76% |", text)
            self.assertIn("### Golden Set Winner Summary", text)
            self.assertIn("| Golden Set Page | Family ID | Parameter Set ID | Baseline | Winner | Δ IoU | Status |", text)
            self.assertIn("| 1 | `unknown` | `winner` | 0.9000 | 0.9700 | +0.0700 | Improved |", text)
            ...
            self.assertIn("| 5 | `unknown` | `winner` | 0.4000 | Poor match; Regressed |", text)
            ...
            self.assertIn("| Classification | Canonical HTH criterion | Engineering interpretation |", text)
            self.assertLess(text.index("#### Parameter Influence"), text.index("#### Parameter Classification"))
            self.assertNotIn("### Canonical Search-Space Accounting", text)
    ...
    def test_builds_combined_manifest_for_multiple_detectors(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("| Runner profile | GitHub hosted — 4 vCPU / 8 threads per detector regression |", text)
            ...
            self.assertIn("| 1 | `grabcut` | 12s | 8 |", text)
            self.assertIn("| 2 | `contour` | 8s | 8 |", text)
            ...
            self.assertIn("#### Estimated Runtime by Search Scope", text)
    ...
    def test_best_known_calibration_build_link_and_persistent_record_footnote(self):
        ...
        text = "\n".join(lines)
        self.assertIn("| Role | Golden Set ID | Date | Build* | Est. Serial Runtime** | Family ID | Parameter Set ID | Parameter Sets | Search Type | Successful Parameter Sets |", text)
        self.assertIn("| Calibration Evidence | Approval Level |", text)
