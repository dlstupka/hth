...
class RegressionSummaryTests(unittest.TestCase):
    def test_builds_manifest_with_winner_baseline_and_outputs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("| Rank | Detector | Detector ID | Role | Golden Set ID | Date | Build* | Run Time** | Parameter Set ID | Parameter Sets | Search Type | Successful Parameter Sets |", text)
    ...
    def test_best_known_calibration_build_link_and_persistent_record_footnote(self):
        ...
        text = "\n".join(lines)
        self.assertIn("| Role | Golden Set ID | Date | Build* | Run Time** | Parameter Set ID | Parameter Sets | Search Type | Successful Parameter Sets |", text)
        self.assertIn("| Calibration Evidence | Approval Level |", text)
