...
class RegressionSummaryTests(unittest.TestCase):
    def test_builds_manifest_with_winner_baseline_and_outputs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("| Result | Golden Set ID | Detector Config ID* | Parameter Set ID | Parameter Short Name |", text)
            self.assertIn("| Winner | `HTH-0001` | `unknown` | `winner` | `calibrated-winner` | 0.9700", text)
            ...
            self.assertIn("| Rank | Last Build | Parameter Set ID | Parameter Short Name | Avg IoU | Min IoU | StdDev | Δ Avg IoU | Avg IoU Success | Failures | Discovery Time | Search Space % |", text)
            ...
            self.assertIn("| Baseline | `HTH-0001` | `unknown` | `base` | `baseline` | 0.9000", text)
