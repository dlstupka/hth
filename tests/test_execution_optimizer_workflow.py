...
class ExecutionOptimizerWorkflowTests(unittest.TestCase):
    ...
    def test_execution_optimizer_serially_reuses_normal_regression_driver(self) -> None:
        ...
        self.assertNotIn('export SHARDS="$pipelines"', text)
        self.assertIn("SHARDING: ${{ inputs.sharding }}", text)
    ...
    def test_execution_optimizer_records_shards_and_current_run_only_reports(self) -> None:
        ...
        shape_run = text.index("bash hth-pipeline/tools/run-detector-regressions.sh")
        shape_finish = text.index('shape_finished="$(date +%s)"', shape_run)
        shape_replay = text.index('--replay-shard-log "$shard_log"', shape_run)
        self.assertLess(shape_replay, shape_finish)
    ...
    def test_manual_workflows_offer_one_canonical_runner_target(self) -> None:
        ...
        for name in (
            ...
            self.assertIn("- 192vcpu", text, name)
            self.assertIn("- 96vcpu", text, name)
    ...
    def test_capacity_runner_uses_catalog_label_and_detected_budget(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("inputs.runner_target == '192vcpu'", text)
        self.assertIn("fromJSON('[\"self-hosted\",\"Linux\",\"X64\",\"192vcpu\"]')", text)
