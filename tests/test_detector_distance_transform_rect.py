import cv2
import numpy as np
from hth.geometry import detector_distance_transform_rect
...
def test_detector_contract():
    ...
    candidate=detector_distance_transform_rect.detect(image_bgr=image,mask=mask)
    assert candidate.method == "distance_transform_rect"
    assert candidate.status in ("ok","no_candidate")

def test_unknown_parameter_rejected():
    image=np.zeros((100,100,3),np.uint8); mask=np.zeros((100,100),np.uint8)
    try: detector_distance_transform_rect.detect(image_bgr=image,mask=mask,parameters={"mystery":1})
