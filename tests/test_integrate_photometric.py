...
class PhotometricIntegrationTests(unittest.TestCase):
    ...
    def test_workflow_exists_and_publishes_durable_release(self) -> None:
        ...
        self.assertIn("canonical_build_policy:", workflow)
        self.assertIn("--scope hth-photometric-integration", workflow)
        self.assertIn("python -m hth.canonical_build_evidence prepare", workflow)
        self.assertIn("python -m hth.canonical_build_evidence finalize", workflow)
        self.assertIn("python -m hth.canonical_build_evidence merge", workflow)
        ...
        policy_position = workflow.index(
            ...
            "photometric-integrated-collection/applied-integration-policy.json"
        )
        finalize_position = workflow.index("- name: Finalize photometric Canonical Build Evidence")
        self.assertLess(package_position, policy_position)
        self.assertLess(policy_position, finalize_position)
        self.assertIn("source hth-pipeline/tools/hardened-release.sh", workflow)
        self.assertIn("hth_publish_immutable_release", workflow)
