...
from hth.geometry.detector_contour_components import BASELINE_PARAMETERS, detect


class ContourComponentsDetectorTests(unittest.TestCase):
    def test_detects_component_supported_contour_quadrilateral(self) -> None:
        ...
        self.assertEqual(candidate.method, "contour_components")
        self.assertEqual(candidate.status, "ok")
        self.assertIsNotNone(candidate.corners)
        self.assertEqual(candidate.diagnostics["parameters"], BASELINE_PARAMETERS)
        self.assertGreater(candidate.diagnostics["component_score"], 0.05)

    def test_rejects_unknown_parameter(self) -> None:
        image = np.zeros((100, 100, 3), dtype=np.uint8)
        mask = np.zeros((100, 100), dtype=np.uint8)
        with self.assertRaisesRegex(ValueError, "Unknown Contour \\+ Components parameters"):
            detect(image_bgr=image, mask=mask, parameters={"mystery": 1})

    def test_regression_config_defines_baseline_profile(self) -> None:
        path = Path(__file__).resolve().parents[1] / "config" / "detectors" / "contour_components.json"
        config = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(config["detector"], "contour_components")
