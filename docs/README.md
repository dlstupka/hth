...
- The manual `HTH execution optimizer` workflow enumerates pipeline/shard shapes for one detector. Auto mode uses representative pipeline counts through the selected runner budget; manual mode evaluates every pipeline count in an inclusive minimum/maximum range. Child regressions use `threads=auto`, so the aggregate runner budget is divided across the active pipelines. After the experiments, the workflow publishes `optimizer-index.json`, `execution-optimizer/<detector>/summary.md`, and `execution-optimizer/<detector>/heatmap.svg` to the results repository and displays the table and heat map in the job summary. It validates the selected runner with the normal regression environment/toolchain checks, releases that runner, and uses a GitHub-hosted controller with a UTC heartbeat to dispatch and monitor child regressions without occupying the target self-hosted runner.
