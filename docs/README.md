...
- `parallelism-index.json` preserves raw compatible execution-shape observations, compatibility-scoped best shapes, and per-shape aggregate statistics including shard count, active pipelines, threads per pipeline, allocated threads, wall time, effective acceleration, parallel efficiency, throughput, workload identity, and concrete runner identity.
- `optimizer-index.json` is a derived execution-planning abstraction built from `parallelism-index.json`. It groups compatible shapes by detector and concrete runner profile, preserves per-runner best shapes, and drives the persistent optimizer summary table and multi-runner heat map.
- The manual `HTH execution optimizer` workflow runs as one long-lived job on the selected runner and serially evaluates pipeline/shard execution shapes for one detector. Auto mode uses representative pipeline counts through the selected runner budget; manual mode evaluates every pipeline count in an inclusive minimum/maximum range. For each shape, shards equal pipelines and `threads=auto` divides the aggregate runner budget across the active pipelines. Every shape invokes the same shared detector-regression execution script used by the normal regression workflow, so queueing, sharding, lifecycle telemetry, heartbeat context, and detector output remain directly comparable. Optimizer runs do not publish calibration intelligence, regression manifests, or normal regression artifacts; they persist only `parallelism-index.json`, derived `optimizer-index.json`, `execution-optimizer/<detector>/summary.md`, and `execution-optimizer/<detector>/heatmap.svg`. The selected runner, Python/ABI/toolchain/OpenCV checks, OpenCV benchmark, pipeline enumeration, and the serial experiment loop are all provided by `./.github/workflows/_core-hth.yml`.
