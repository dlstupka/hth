...
- `parallelism-index.json` preserves raw compatible execution-shape observations, compatibility-scoped best shapes, and per-shape aggregate statistics including shard count, active pipelines, threads per pipeline, allocated threads, wall time, effective acceleration, parallel efficiency, and throughput.
- The manual `HTH execution optimizer` workflow enumerates pipeline/shard shapes for one detector. Auto mode uses representative pipeline counts through the selected runner budget; manual mode evaluates every pipeline count in an inclusive minimum/maximum range. Child regressions use `threads=auto`, so the aggregate runner budget is divided across the active pipelines.
