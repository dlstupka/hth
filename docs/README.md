...
- The manual `HTH execution optimizer` workflow enumerates pipeline/shard shapes for one detector. Auto mode uses representative pipeline counts through the selected runner budget; manual mode evaluates every pipeline count in an inclusive minimum/maximum range. Child regressions use `threads=auto`, so the aggregate runner budget is divided across the active pipelines. After the experiments, the workflow publishes `optimizer-index.json`, `execution-optimizer/<detector>/summary.md`, and `execution-optimizer/<detector>/heatmap.svg` to the results repository and displays the table and heat map in the job summary. Selected-runner validation and pipeline enumeration are provided by `./.github/workflows/_core-hth.yml`, including the established Python, ABI, toolchain, OpenCV, benchmark, and heartbeat checks. The selected runner is then released while a GitHub-hosted controller dispatches and monitors child regressions without occupying the target self-hosted runner. The controller performs GitHub CLI dispatch from the checked-out `hth-pipeline` repository so repository-scoped workflow commands resolve correctly.
