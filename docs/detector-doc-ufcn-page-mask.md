...
### Damaged-spread outer-background proof

For a vertically trustworthy single learned leaf that leaves a large horizontal portion of a spread unexplained, Doc-UFCN completion uses two independent source-image proofs. Robust vertical-gradient peaks remain the primary boundary evidence. A faint or torn physical edge may not dominate that gradient profile, so HTH also measures robust column luminance and can accept the outermost sustained transition from paper into stable scanner background. This secondary proof is available only after the existing damaged-spread gates qualify; it does not expand ordinary single-page masks or relax the learned-component envelope rules.
