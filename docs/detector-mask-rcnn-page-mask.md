...
## Manual model variants

Manual detector regression builds expose a generic **Model variant** selector. `default` resolves to the detector's current registered model; detectors without registered variants treat it as N/A. Mask R-CNN currently registers `rcnn_hjdataset_current` and `rcnn_hjdataset_older`. The current variant uses the LayoutParser Hugging Face mirror, while the older variant resolves the original HJDataset Mask R-CNN artifact/config URLs from LayoutParser's legacy model catalog. Each variant is stored under a distinct model id with immutable SHA-256 provenance, and the active variant/model id is recorded in the regression calibration identity. Historic-best lookup is variant-aware so an experimental older model cannot inherit the current model's best parameter set.

The registry is intentionally detector-agnostic (`hth/model_variants.py`): future neural detectors can add named model variants without turning model choice into a calibration parameter. Automatic builds remain on `default`, so model A/B experiments are opt-in and do not multiply CI coverage.
