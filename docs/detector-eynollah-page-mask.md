...
The detector lifecycle downloads the released TensorFlow SavedModel into the runner-local model cache, records each artifact SHA-256 plus the source/reference that supplied it, and exposes the prepared model to the regression worker. Model payloads are never results-repository content. Golden Set inference is performed once in the parent learned-evidence stage before pipeline fan-out; calibration therefore changes only deterministic post-inference thresholding, page-area rejection, morphology, and final page-envelope padding.
