...
`HTH-0001` is the original five-page calibration/validation set (global ordinals 1, 5, 6, 9, and 10) and is now frozen. Its existing `config/golden_set.json` bytes are intentionally unchanged so all previously persisted calibration evidence remains compatible with the same Golden Set SHA-256. It is the sole legacy short-form identity; new sets use their immutable release tag verbatim as the Golden Set ID, beginning with `HTH-GOLDEN-0002`.

The external freeze manifest at `config/golden_sets/HTH-0001.freeze.json` records the immutable content hash, membership, source-document identity, and freeze policy. CI runs `hth/validate_golden_set_freeze.py` whenever the Golden Set or freeze metadata changes. Any in-place change to HTH-0001 fails validation; new or corrected truth data must receive a new Golden Set identity matching its release tag, such as `HTH-GOLDEN-0002`.
...
- `HTH-GOLDEN-0002.golden-set.json`, exported by the multidetector reference editor;
- `HTH-GOLDEN-0002.freeze.json`, exported immediately afterward from the same approved editor state; and
- `HTH-GOLDEN-0002.images.zip`, a deterministic bundle containing only the exact approved processed images; and
...
1. Expand the SOURCE-0002 production review artifact and open `tools/reference-collection-editor.html`. The editor defaults to GS0002; select a different Golden Set JSON or a new source draft when reviewing another collection. Images load on demand.
...
6. Enter `HTH-GOLDEN-0002` plus creator, reviewer, approver, and change-note provenance. Choose **Approve current state**. This exact value becomes the JSON identity, freeze identity, release tag, and asset filename prefix.
