...
python -m hth.regress_detector \
  --detector-config config/detectors/ransac.json  # or components.json / contour.json / grabcut.json \
...
For a manual run, the **Algorithm** input is a choice of `all`, `contour`, `components`, `ransac`, or `grabcut`. Automatic smoke runs continue to exercise all configured detector algorithms.
