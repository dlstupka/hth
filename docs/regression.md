...
Inside a multi-detector regression job, HTH builds the complete pipeline schedule before worker fan-out. Detector runtimes are ordered by Longest Processing Time (LPT), then each detector is greedily assigned to the pipeline with the least accumulated estimated work. This makes the longest jobs gate the predicted makespan while shorter work fills the remaining pipeline capacity. The resulting per-pipeline queues are fixed for the run: workers do not steal, refill, or dynamically reclaim detector work after startup.

Regression results accumulate locally during the fixed schedules and calibration/index persistence is published after the run as one batched results transaction, avoiding persistence contention between active pipelines.

Every scheduled detector emits UTC-timestamped `LOAD`, `START`, and `UNLOAD` lifecycle lines with detector ID, shard identity, thread count, status, and wall time. Unsharded work is shown consistently as shard `1/1`. A configured pipeline stagger is applied after `LOAD` and before `START`, preserving the distinction between queue residence and detector execution.
...
- a **Best Known Detector Calibrations** table that first selects the newest detector implementation revision represented by persisted evidence, then prefers compatible full calibrations within that revision and falls back to smoke/partial evidence when the revised implementation has not yet completed a full calibration. This prevents an older exhaustive result from masquerading as the calibration of detector code that has since changed. The table records rank, detector ID, Golden Set ID, calibration date, a compact linked GitHub Actions build number, estimated single-detector serial runtime, parameter set ID, parameter-set count, search type, success rate, winner metrics, baseline delta, basin width, equivalent-best coverage, deterministic Calibration Evidence, and automatic Golden Set-scoped Approval Level; the redundant Coverage column is omitted, build run time is the wall-clock duration recorded for that build, and build links are temporary operational shortcuts while the persisted `calibration-intelligence.json` remains authoritative;
...
Every registered learned detector resolves deterministic parameter-invariant evidence through the collection-scoped canonical cache. Compatible identities first use a runner-local bundle, then an immutable cache release; misses regenerate and populate both tiers. Single-task detectors resolve inside their assigned pipeline, while shard/lane consumers and detectors sharing the same neural evidence prepare it once. See [Canonical learned-evidence cache](learned-evidence-cache.md).
...
- **Ranked Detector Smoke Test Results** uses the latest persisted `provisional` smoke observation for each detector and reports the best result actually evaluated by that smoke run. Mandatory Baseline and Historic Best references participate in winner selection alongside the requested smoke-search members, so a smoke result cannot rank below the compatible Historic Best that it just reevaluated; a requested smoke member may still win when it genuinely improves on both references. Parameter count remains the declared smoke-search size, while winning identity, quality, timing, and rate come from that run's actual merged winner. The separate **Best Known Detector Calibrations** table continues to report authoritative persisted calibration history.
