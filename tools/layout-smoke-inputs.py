#!/usr/bin/env python3
"""Materialize verified source/final-normalized pairs for layout reconnaissance.
...
def materialize(
    ...
    photometric = _load(photometric_manifest_path)
    methods = _load(method_assessment_path)
    final = _load(final_manifest_path)
    ...
    if len(results_commit) != 40 or any(c not in "0123456789abcdef" for c in results_commit):
    ...
    if not ordinals or not all(n in source_images and n in base_pages and n in photometric_pages and n in final_pages for n in ordinals):
        raise ValueError("Golden Set page is missing from source or normalization evidence")
    ...
    for ordinal in ordinals:
        ...
        if image is None or _pixel_sha256(image) != base_record["source_pixel_sha256"]:
        ...
        rows.append({
            ...
            "source_pixel_sha256": base_record["source_pixel_sha256"],
    ...
    result = {
        "schema_version": "1.0",
        "purpose": "layout-smoke-paired-inputs",
        "golden_set_id": freeze["golden_set_id"],
        "golden_set_sha256": freeze["golden_set_sha256"],
        "source_release": freeze["source_release"],
        "results_commit": results_commit,
        "base_normalization_result_identity": base["canonical_result_identity"],
        "final_normalization_result_identity": final["binarization_result_identity"],
        "pages": rows,
    }
...
def main() -> None:
    ...
    result = materialize(
        ...
        args.final_manifest, args.output, args.results_commit,
    )
    print(f"Verified {len(result['pages'])} paired GS images at {args.output}")
